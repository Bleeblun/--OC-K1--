...
#define DEFAULT_S2W_X_MAX               1200
#define DEFAULT_S2W_Y_LIMIT             DEFAULT_S2W_Y_MAX-200
#define DEFAULT_S2W_X_B1                200
#define DEFAULT_S2W_X_B2                600
...
static ssize_t s2w_screen_suspended_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%i\n", s2w_scr_suspended);
}

static ssize_t s2w_screen_suspended_dump(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int data;
	if(sscanf(buf, "%i\n", &data) == 1)
		s2w_scr_suspended = data;
	else
		pr_info("%s: unknown input!\n", __FUNCTION__);
	return count;
}

static DEVICE_ATTR(s2w_screen_suspended, (S_IWUSR|S_IRUGO),
	s2w_screen_suspended_show, s2w_screen_suspended_dump);
...
static int __init sweep2wake_init(void)
{
	...
	rc = sysfs_create_file(android_touch_kobj, &dev_attr_s2w_screen_suspended.attr);
	if (rc) {
		pr_warn("%s: sysfs_create_file failed for s2w_screen_suspended\n", __func__);
	}
